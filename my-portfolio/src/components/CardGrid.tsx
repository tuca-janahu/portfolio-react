...
function CardGrid({
  ...
  return (
    <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {list.map((p) => (
        <SpotlightCard key={p.id} className="p-4 sm:p-5">
          <div className="flex h-full flex-col">
            ...
            {p.href && (
              <a
                href={p.href}
                target="_blank"
                className="w-full sm:w-auto inline-flex mt-6 sm:mt-8 items-center justify-center rounded-xl border border-black/10 dark:border-white/10 px-5 py-2.5 text-sm hover:bg-black/5 dark:hover:bg-white/5"
              >
                Ver projeto
              </a>
            )}
